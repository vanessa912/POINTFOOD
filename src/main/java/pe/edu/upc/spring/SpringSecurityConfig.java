	...
	protected void configure(HttpSecurity http) throws Exception {
		try {
			http.authorizeRequests()
				...
				.antMatchers("/plato/**").access("hasRole('ROLE_ADMIN')")
				.antMatchers("/mesa/**").access("hasRole('ROLE_ADMIN')")
				.antMatchers("/ingrediente/**").access("hasRole('ROLE_ADMIN')")
				.antMatchers("/reserva/**").access("hasRole('ROLE_ADMIN') or hasRole('ROLE_USER')").and()
				.formLogin().successHandler(successHandler).loginPage("/login").loginProcessingUrl("/login").defaultSuccessUrl("/cliente/bienvenido")
